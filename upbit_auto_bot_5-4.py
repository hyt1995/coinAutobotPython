...
#내가 매수할 총 코인 개수
MaxCoinCnt = 5.0    #수정 가능한 부분: 6

#처음 매수할 비중(퍼센트)
FirstRate = 10.0   #수정 가능한 부분: 4
#추가 매수할 비중 (퍼센트)
WaterRate = 5.0   #수정 가능한 부분: 5
...
for ticker in Tickers:
    try: 
        ...
        df_60 = pyupbit.get_ohlcv(ticker,interval="minute60") #60분봉 데이타를 가져온다.  수정 가능한 부분: 1

        #RSI지표를 구한다.
        #제가 현재 캔들을 -1 이 아니라 -2로 
        #이전 캔들을 -2가 아니라 -3으로 수정했습니다. 이유는 아래 이어지는 주석의 내용을 참고하세요!
        rsi60_min_before = GetRSI(df_60,14,-3) #수정 가능한 부분: 2
        rsi60_min = GetRSI(df_60,14,-2) #수정 가능한 부분: 3
        ...
        # 현재 밑에 있는 코인들은 매수한 코인들이다 .
        # 여기서 하락장일 경우 특정 %를 매도하는 코드를 짠다.
        if IsHasCoin(balances,ticker) == True:
            ...
            #수정 가능한 부분: 8 - and 를 or로 바꾸면 70이상이거나 수익권이 1%만 나도 분할매도를 하게 된다.
            if rsi60_min >= 70.0 and revenu_rate >= 1.0:
                ...
                #최대코인매수금액의 1/4보다 크다면 절반씩 시장가 매도 
                    ...
                    print(upbit.sell_market_order(ticker,upbit.get_balance(ticker) / 2.0))
                     #수정 가능한 부분: 7 - 분할매도 하지않고 한번에 매도할때

            #내가 가진 원화가 물탈 돈보다 적다..(원금 바닥) 그런데 수익율이 - 10% 이하다? 그럼 절반 팔아서 물탈돈을 마련하자!
            #수정 가능한 부분: 9 - 수익율이 몇%일때 매도를 통해서 물탈돈을 마련할것이가 하락장일 경우 코인가격이 계속 내려가기 때문에 필요한 부분
            ...
            #수정 가능한 부분: 10 - 코인 매수시 rsi지표가 30 이 넘어가면서 상승세일경우 매수
            if rsi60_min_before <= 30.0 and rsi60_min > 30.0:
                ...
                #50%를 초과하면
                    ...
                    #수정 가능한 부분: 11 - 하락장일 경우 매수를 진행할때 수익율이 -몇%일경우에 매수를 통해 물타기를 할것인가에 대한 
                    if revenu_rate <= -5.0:
                        time.sleep(0.05)
                        print(upbit.buy_market_order(ticker,WaterEnterMoeny))
        ...
        #아직 매수하기 전인 코인들 즉 매수 대상
            ...
            #수정 가능한 부분: 12 - rsi지표가 30이상으로 상승세일때 매슈를 진행하는 것이다 그게 싫으면 바로 위에 if문 주석해제
...
# 하락장일 경우 rsi지표 30이하에 매수를 하고 70이상에 팔아도 근본적으로 가격이 내려가고 있기때문에 손해가 난다.
# 그래서 금액이 몇%손실일경우 파는 코드가 있어야하는데
# 아니면 rsi지표를 떠나서 처음 매수한 금액의 몇%이하로 떨어지면 일단 다 매도를 한다도 전략이 될수있다.
# 지금 여기서 소개하는 존버봇은 하락장일 경우 매수한 금액의 특정 %이하로 떨어지면 특정%(50%)를 매도하고 
# 그 매도한 금액으로 다시 rsi30이하 저점에서 다시 매수를 한다. 그렇게 물을타며 상승일때 팔겠다는 것 같다.
# 아니면 
# 하락장을 대비해 rsi지표 30만 고려하고 수익이 5%이상이면 팔겠다라는것을 전략으로 삼고가도 된다.
# 그래서 현재 코딩하고 있는 존벗봇은 rsi 지표가 70이상일때 수익률이 1%이상이라도 있으면 그때 다 매도를 하겟다.
# 위에 있는 경우가 아닐경우 존버를한다.




# crontap 이 돌때마다 메일을 보내는 기능이 있는데 막는것은 간단하다.
# MAILTO=""
