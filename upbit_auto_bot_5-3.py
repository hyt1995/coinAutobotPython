...
access = "A0X27AGgl8UYAC2cFMYzyrMlfxn1DsgrxoGjLVc2"          # 본인 값으로 변경  - 이미 변경함
secret = "JkaADmlhsKOAcOlSsYw1WJ7DIpSnM9gGzP7dLRBx"          # 본인 값으로 변경 - 이미 변경함
...
# 현재 밑에있는 코드는 
# 거래량이 많은 코인들중에 top 5를 가져온 후 
# 60분봉을 기준으로 해서 
# rsi14 지표가 30이하로 떨어졌을때만 매수를 진행하는 코드이다.
# 15or 10분씩 봇을 돌려서 분할매수를 진행할것이다.
...
for ticker in Tickers:
    try: 
        ...
        #이렇게 쉬어주는거 잊지 마세요!
        time.sleep(0.05)
        #밑에 분봉정보를 가져왔기때문에 이정도는 쉬어주어야 데이터를 안전하게 온전하게 가져온다고 한다.
        ...
        if IsHasCoin(balances,ticker) == True:
            ...
            if rsi60_min <= 30.0:
                ...
                #50%를 초과하면
                    ...
                    # 한마디로 분할매수를 할때 정한 금액만큼 매수를 하는데 정한 금액을 넘어갈을경우 수익률이 마이너스이면 그때 매수를 진행한다.
                    if revenu_rate <= -5.0:
                        time.sleep(0.05)
                        print(upbit.buy_market_order(ticker,WaterEnterMoeny))
